...
class ComApi(api.ApiHandler):
    ...
    def IWbemLocator_ConnectServer(self, emu, argv, ctx={}):
        ...
        if ppNamespace:
            ...
            pNamespace = self.mem_alloc(
                emu.get_ptr_size(), tag="emu.COM.ppNamespace_IWbemServices"
            )
            self.mem_write(
                pNamespace, ci.address.to_bytes(emu.get_ptr_size(), "little")
            )
            self.mem_write(
                ppNamespace, pNamespace.to_bytes(emu.get_ptr_size(), "little")
            )
