    ...
    def __init__(self, api, family, stype, protocol, flags):
        ...
        if flags == WSK_FLAG_DATAGRAM_SOCKET:
            ...
            addr = self.emu.add_callback(
                Netio.name, api.WskControlSocket.__apihook__[0]
            )  # noqa
            self.dispatch.Basic.WskControlSocket = addr
            addr = self.emu.add_callback(
                Netio.name, api.WskCloseSocket.__apihook__[0]
            )  # noqa
            ...
            addr = self.emu.add_callback(
                Netio.name, api.WskSendTo.__apihook__[0]
            )  # noqa
            self.dispatch.WskSendTo = addr
            addr = self.emu.add_callback(
                Netio.name, api.WskReceiveFrom.__apihook__[0]
            )  # noqa
            self.dispatch.WskReceiveFrom = addr
            addr = self.emu.add_callback(
                Netio.name, api.WskRelease.__apihook__[0]
            )  # noqa
            self.dispatch.WskRelease = addr
            addr = self.emu.add_callback(
                Netio.name, api.WskGetLocalAddress.__apihook__[0]
            )  # noqa
...
class Netio(api.ApiHandler):
    ...
    def __init__(self, emu):
        ...
        addr = emu.add_callback(
            Netio.name, self.WskSocketConnect.__apihook__[0]
        )  # noqa
        self.prov_disp.WskSocketConnect = addr
        addr = emu.add_callback(
            Netio.name, self.WskControlClient.__apihook__[0]
        )  # noqa
        self.prov_disp.WskControlClient = addr
        addr = emu.add_callback(
            Netio.name, self.WskGetAddressInfo.__apihook__[0]
        )  # noqa
        self.prov_disp.WskGetAddressInfo = addr
        addr = emu.add_callback(
            Netio.name, self.WskFreeAddressInfo.__apihook__[0]
        )  # noqa
    ...
    def WskCaptureProviderNPI(self, emu, argv, ctx={}):
        ...
        if not cli:
            ...
            if not self.provider_npi:
                ...
                wpd_addr = self.mem_alloc(
                    size=self.sizeof(self.prov_disp),  # noqa
