...
class Ndis(api.ApiHandler):
    ...
    def NdisAllocateGenericObject(self, emu, argv, ctx={}):
        ...
        ptr = self.mem_alloc(
            size=total, tag="api.struct.NDIS_GENERIC_OBJECT.%s" % (stag)
        )
    ...
    def NdisAllocateNetBufferListPool(self, emu, argv, ctx={}):
        ...
        params = self.mem_cast(
            self.ndis.NET_BUFFER_LIST_POOL_PARAMETERS(emu.get_ptr_size()), Parameters
        )
        ...
        if params.fAllocateNetBuffer:
            ...
            nb_ptr = self.mem_alloc(
                size=self.sizeof(net_buf), tag="api.struct.NET_BUFFER"
            )
            nbl.FirstNetBuffer = nb_ptr

            self.mem_write(nb_ptr, self.get_bytes(net_buf))

        nbl_ptr = self.mem_alloc(
            size=self.sizeof(nbl), tag="api.struct.NET_BUFFER_LIST"
        )
