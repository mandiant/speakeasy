...
class Ntoskrnl(api.ApiHandler):
    ...
    def IoCreateDevice(self, emu, argv, ctx={}):
        ...
        dev = emu.create_device_object(
            name, driver_obj, ext_size, devtype, chars, tag="api.object"
        )

        self.mem_write(
            out_addr, dev.address.to_bytes(self.get_ptr_size(), byteorder="little")
        )
    ...
    def ZwQuerySystemInformation(self, emu, argv, ctx={}):
        ...
        if sysclass == ddk.SYSTEM_INFORMATION_CLASS.SystemModuleInformation:
            ...
            raise ApiEmuError("Unsupported information class: 0x%x" % (sysclass))
    ...
    def PsCreateSystemThread(self, emu, argv, ctx={}):
        ...
        handle, obj = self.create_thread(
            start, startctx, proc_obj, thread_type="system_thread"
        )
    ...
    def PsLookupProcessByProcessId(self, emu, argv, ctx={}):
        ...
        if ProcessId == 4:
            ...
            proc.ref_cnt += 1
            self.mem_write(
                Process, proc.address.to_bytes(self.get_ptr_size(), "little")
            )  # noqa
            return rv
    ...
    def IoGetDeviceObjectPointer(self, emu, argv, ctx={}):
        ...
        if obj:
            if obj.file_object:
                self.mem_write(
                    pFileObject,
                    obj.file_object.address.to_bytes(self.get_ptr_size(), "little"),
                )  # noqa
            self.mem_write(
                pDeviceObject, obj.address.to_bytes(self.get_ptr_size(), "little")
            )
            rv = ddk.STATUS_SUCCESS
    ...
    def RtlDecompressBuffer(self, emu, argv, ctx={}):
        ...
        if fmt not in (
            ddk.COMPRESSION_FORMAT_LZNT1,
            ddk.COMPRESSION_FORMAT_XPRESS,
        ):  # noqa
    ...
    def ObReferenceObjectByHandle(self, emu, argv, ctx={}):
        ...
        if obj:
            if Object:
                self.mem_write(
                    Object, obj.address.to_bytes(self.get_ptr_size(), "little")
                )  # noqa
        else:
    ...
    def ZwQueryValueKey(self, emu, argv, ctx={}):
        ...
        if key:
            ...
            if val:
                ...
                if (
                    info_class
                    == regdefs.KEY_VALUE_INFORMATION_CLASS.KeyValuePartialInformation
                ):
                    ...
                    raise ApiEmuError(
                        "Unsupported information class: 0x%x" % (info_class)
                    )
    ...
    def ZwCreateFile(self, emu, argv, ctx={}):
        ...
        if obj:
            ...
            self.log_file_access(name, "open", disposition=cd, access=ad)
            ...
            self.log_file_access(npath, "open", disposition=cd, access=ad)
    ...
    def ZwOpenFile(self, emu, argv, ctx={}):
        ...
        self.log_file_access(path, "open", disposition=None, access=ad)
    ...
    def ZwQueryInformationFile(self, emu, argv, ctx={}):
        ...
        if _file and FileInformation:
            if (
                ...
                == ddk.FILE_INFORMATION_CLASS.FileStandardInformation
            ):
    ...
    def RtlQueryRegistryValuesEx(self, emu, argv, ctx={}):
        ...
        relatives = {
            regdefs.RTL_REGISTRY_ABSOLUTE: "",  # noqa
    ...
    def ZwWriteFile(self, emu, argv, ctx={}):
        ...
        if _file and buf and length:
            ...
            if data:
                _file.add_data(data)
                # Log the file event
                self.log_file_access(path, "write", data=data, size=length)
    ...
    def ZwCreateSection(self, emu, argv, ctx={}):
        ...
        self.mem_write(
            SectionHandle, hmap.to_bytes(self.get_ptr_size(), byteorder="little")
        )
        argv[0] = hmap
    ...
    def ZwMapViewOfSection(self, emu, argv, ctx={}):
        ...
        if ViewSize:
            bytes_to_map = int.from_bytes(
                self.mem_read(ViewSize, emu.get_ptr_size()), "little"
            )

        pref_address = int.from_bytes(
            self.mem_read(BaseAddress, emu.get_ptr_size()), "little"
        )
        ...
        if sect:
            ...
            if f and not pref_address:
                ...
                if ViewSize:
                    self.mem_write(
                        ViewSize, size.to_bytes(self.get_ptr_size(), "little")
                    )
                argv[2] = buf
                argv[6] = size
                ...
                buf = self.mem_alloc(
                    base=base, size=size, perms=access, shared=True, process=proc_obj
                )
                mm = emu.get_address_map(buf)
