        ...
        for field in fields:
            ...
            if issubclass(field_obj, ctypes.Structure):
                if field_name not in comdefs.IFACE_TYPES:
                    raise Win32EmuError(
                        "COM interface %s inherits unsupported interface %s"
                        % (name, field_name)
                    )
                ...
                for subfield in field_obj._fields_:
                    ...
                    if issubclass(
                        subfield_type,
                        (ctypes.c_uint32, ctypes.c_ulong, ctypes.c_ulonglong),
                    ):
                        ...
                        if hasattr(com_api.ComApi, method_name):
                            ...
                            addr = emu.add_callback(
                                com_api.ComApi.name, method.__apihook__[0]
                            )
                            emu.mem_write(
                                com_ptr + field_offset,
                                addr.to_bytes(emu.get_ptr_size(), "little"),
                            )

                    field_offset += ptr_size
            elif issubclass(
                field_obj, (ctypes.c_uint32, ctypes.c_ulong, ctypes.c_ulonglong)
            ):
                ...
                raise Win32EmuError(
                    "Invalid field type encountered for %s.%s" % (name, field_name)
                )
