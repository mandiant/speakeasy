...
class ApiHammer:
    ...
    def __init__(self, emu):
        ...
        self.allow_list = _lowercase_set(
            self.config.get("allow_list", _default_api_hammer_allowlist)
        )
    ...
    def handle_import_func(self, imp_api, conv, argc):
        ...
        if self.emu.get_arch() == e_arch.ARCH_X86:
            ...
            self.emu.log_info(
                "api hammering at: %s 0x%x %r %r %r"
                % (imp_api, self.emu.get_pc(), mnem, op, instr)
            )
            if (mnem == "call") and "dword ptr" in instr:
                if conv == e_arch.CALL_CONV_CDECL:
                    ...
                    self.emu.log_info(
                        "API HAMMERING DETECTED - patching 1 cdecl at %x" % (eip,)
                    )
                elif conv == e_arch.CALL_CONV_STDCALL:
                    # If stdcall, we need to clean the stack
                    # patch is xor eax, eax; add esp, <count>
                    patch = (
                        b"\x31\xc0\x83\xc4" + (4 * argc).to_bytes(1, "little") + b"\x90"
                    )
                    self.emu.mem_write(eip, patch)
                    self.emu.log_info(
                        "API HAMMERING DETECTED - patching 1 stdcall at %x" % (eip,)
                    )
            else:
                eip = self.emu.get_ret_address() - 2
                mnem, op, instr = self.emu.get_disasm(eip, DISASM_SIZE)
                self.emu.log_info(
                    "api hammering at: 0x%x %r %r %r"
                    % (self.emu.get_pc(), mnem, op, instr)
                )
                if (mnem == "call") and op in e_arch.REG_LOOKUP.keys():
                    ...
                    if self.hammer_memregion is None:
                        self.hammer_memregion = self.emu.mem_map(
                            0x1024 * 4, tag="speakeasy.hammerpatch"
                        )
                    if conv == e_arch.CALL_CONV_CDECL:
                        # If cdecl, the emu engine will clean the stack
                        ...
                        self.emu.mem_write(eip, patch)
                        self.emu.log_info(
                            "API HAMMERING DETECTED - patching 2 cdecl at %x" % (eip,)
                        )  # noqa
                    elif conv == e_arch.CALL_CONV_STDCALL:
                        # patch is xor eax, eax; retn <count>
                        patch = (
                            b"\x31\xc0\xc2" + (4 * argc).to_bytes(2, "little") + b"\x90"
                        )
                        ...
                        if (self.hammer_offset + len(patch)) < 0x1024 * 4:
                            ...
                            self.emu.log_info(
                                "API HAMMERING DETECTED - patching 2 stdcall at %x"
                                % (eip,)
                            )  # noqa
                else:
                    self.emu.log_info(
                        "API HAMMERING DETECTED - unable to patch %x" % (eip,)
                    )
