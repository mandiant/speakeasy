...
class EmuEngine(object):
    ...
    def hook_add(
        self, addr=None, cb=None, htype=None, ctx=None, begin=1, end=0, arg1=0
    ):
        """
        ...
        if hook_type == uc.UC_HOOK_INSN:
            if arg1 == u.UC_X86_INS_IN:  # IN instruction
                cb = ct.cast(
                    unicorn.unicorn.UC_HOOK_INSN_IN_CB(cb),
                    unicorn.unicorn.UC_HOOK_INSN_IN_CB,
                )
            elif arg1 in (
                u.UC_X86_INS_SYSCALL,
                u.UC_X86_INS_SYSENTER,
            ):  # SYSCALL/SYSENTER
                cb = ct.cast(
                    unicorn.unicorn.UC_HOOK_INSN_SYSCALL_CB(cb),
                    unicorn.unicorn.UC_HOOK_INSN_SYSCALL_CB,
                )
        elif hook_type == uc.UC_HOOK_CODE:
            cb = ct.cast(
                unicorn.unicorn.UC_HOOK_CODE_CB(cb), unicorn.unicorn.UC_HOOK_CODE_CB
            )
        elif hook_type in (uc.UC_HOOK_MEM_READ, uc.UC_HOOK_MEM_WRITE):
            cb = ct.cast(
                unicorn.unicorn.UC_HOOK_MEM_ACCESS_CB(cb),
                unicorn.unicorn.UC_HOOK_MEM_ACCESS_CB,
            )
        elif hook_type == uc.UC_HOOK_MEM_INVALID:
            cb = ct.cast(
                unicorn.unicorn.UC_HOOK_MEM_INVALID_CB(cb),
                unicorn.unicorn.UC_HOOK_MEM_INVALID_CB,
            )
        else:
            return self.emu.hook_add(
                htype=hook_type, callback=cb, user_data=ctx, begin=begin, end=end
            )
        ...
        if hook_type == uc.UC_HOOK_INSN:
            insn = ct.c_int(arg1)
            rv = _uc.uc_hook_add(
                handle, ct.byref(hook_id), hook_type, ptr.value, None, begin, end, insn
            )
        else:
            rv = _uc.uc_hook_add(
                handle, ct.byref(hook_id), hook_type, ptr.value, None, begin, end
            )
        if rv != uc.UC_ERR_OK:
            raise uc.UcError(rv)
